...
public class SetupPartsSubclassPageThree {
	//靜態變數宣告
	...
	public static final String PAGE_THREE_SHEET = UtilConfigReader.readProperty("PAGE_THREE_SHEET");
	public static IAgileSession session = null;
	public static AgileSessionFactory factory;
	
	public SetupPartsSubclassPageThree() { }
	    ...
	    while(isFilepathRight==false) {
		    ...
		    try {
			    ...
			    if(XLS_IMPORT.lastIndexOf(".")==-1 || !fileExtension.equals(".xls")) {
				    ...
				    //取得Sheet
				    sheet = workbook.getSheet(PAGE_THREE_SHEET);
			    }
			    
			    /***開始匯入Attributes設定***/
			    //Excel Sheet Schema (Parts.P3)
			    //Column 0:Subclass Name
	    		...
	    		try {
				    ...
				    for (int i=1; i<=sheet.getLastRowNum(); i++) {
			    		...
			    		**/    	
				    	...
				    	INode p3node = admin.getNode(subclassAPIName+".PageThree");
				    	p3node.getProperty(PropertyConstants.PROP_VISIBLE).setValue("Yes");
				    	
				    	session.disableAllWarnings();
				    	
				    	//設定屬性
				    	IAttribute attr = admin.getAgileClass(subclassName).getAttribute("PageThree."+APIName.trim());
				    	//
				        ...
				        //
				    	if(!(inputWidth==null || inputWidth.equals(""))) {
				    	...
				    	**/
				    	...
				    	logger.info("Set subclass: [ " + subclassName + " ]-Attribute Name(APIName): [ " + attrName +"("+ APIName +")" + " ] OK ");
				    }
